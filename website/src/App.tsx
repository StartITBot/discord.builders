        ...
        <div className={Styles.app}>
            ...
            <div className={Styles.json}>
                ...
                <p style={{ marginBottom: '2rem', marginTop: '1rem' }}>
                    Warning: Non-link buttons and select menus are not allowed when sending message via webhook.
                </p>
                ...
                <div className={Styles.input_pair}>
                    ...
                    <button
                        ...
                        onClick={async () => {
                            ...
                            if (status_code === 204)
                                return dispatch(actions.setWebhookResponse({ status: '204 Success' }));
                ...
                {!!response && (<CodeDisplay code={JSON.stringify(response, undefined, 4)} language={'json'}/>)}

                <div className={Styles.tabs}>
                    <div
                        className={Styles.tab + ' ' + ('json' === libSelected ? Styles.active : '')}
                        onClick={() => setLibSelected('json')}
                    >
                        JSON
                    </div>
                    {Object.keys(libComponents).map((comp) => (
                        <div
                            className={Styles.tab + ' ' + (comp === libSelected ? Styles.active : '')}
                            onClick={() => setLibSelected(comp)}
                        >
                            {libs[comp]?.name || comp}
                        </div>
                    ))}
                ...
                <div className={Styles.data}>
                    <CodeDisplay code={data} language={language} />
                </div>
            </div>
        </div>
    );
