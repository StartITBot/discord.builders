...
function GlobalSettings({state, stateKey, stateManager}: {
    ...
    const {t} = useTranslation("components-sdk")
    return <div className={Styles.select_option + ' ' + Styles.select_default + (open ? " " + Styles.open : "")}
                onClick={(ev) => {
        ...
        <div className={Styles.with_badge}>
            ...
            <div
                className={Styles.badge + ' ' + (isInvalid ? Styles.invalid : '')}>{isInvalid && t('string-select.invalid')} {min_values === max_values ? min_values : `${min_values} – ${max_values}`}</div>
        </div>
        {!!open && <div className={CapsuleStyles.large_button_ctx + ' ' + CapsuleStyles.noright} ref={btn_select}>
            {open === 1 &&
                <GlobalSettingsFirst state={state} stateKey={stateKey} stateManager={stateManager} setOpen={setOpen}/>}
            {open === 2 && <MenuLabel closeLockRef={closeLockRef} state={state.placeholder || ""}
                                      stateKey={[...stateKey, 'placeholder']} stateManager={stateManager}
                                      setOpen={setOpen}/>}
            {open === 3 &&
                <MenuRange min={1} max={state.options.length} state={min_values} stateKey={[...stateKey, 'min_values']}
                           stateManager={stateManager}/>}
            {open === 4 &&
                <MenuRange min={1} max={state.options.length} state={max_values} stateKey={[...stateKey, 'max_values']}
                           stateManager={stateManager}/>}
            {open === 5 &&
                <MenuLabel closeLockRef={closeLockRef} state={state.custom_id} stateKey={[...stateKey, 'custom_id']}
                           stateManager={stateManager} setOpen={setOpen} placeholder="custom_id" lockClose={false}/>}
        </div>}
...
function GlobalSettingsFirst({state, stateKey, stateManager, setOpen}: {
    ...
    return <>
        <div className={CapsuleStyles.large_button_ctx_item} onClick={() => {
            ...
            <div className={CapsuleStyles.large_button_ctx_item_img}><img src={state.disabled ? LockActive : Lock}
                                                                          alt=""/></div>
            <div
        ...
        <div className={CapsuleStyles.large_button_ctx_item} onClick={(ev) => {
            ...
            <div
                className={CapsuleStyles.large_button_ctx_item_text}>{!state.placeholder ? t('string-select.add-placeholder') : t('string-select.change-placeholder')}</div>
        </div>
...
function StringSelectOption({
                                state,
                                stateKey: stateParent,
                                index,
                                stateManager,
                                passProps,
                                removeKeyParent,
    ...
    return (
        <DragLines draggable={true} removeKeyParent={removeKeyParent} droppableId={DroppableID.STRING_SELECT}
                   data={state} stateKey={stateKey} dragDisabled={!!open}>
            <div
                className={Styles.select_option + (open ? " " + Styles.open : "") + (state.default ? " " + Styles.blue : "") + (disabled ? " " + Styles.disabled : "")}
                onClick={(ev) => {
                    if (btn_select.current && btn_select.current.contains(ev.target as HTMLElement)) return;
                    setOpen(1)
                }} ref={ignoreRef}>
                ...
                <div className={Styles.text}>
                    ...
                    <span className={Styles.desc}>&bull; {state.description}</span>}
                </div>
                {!!open &&
                    <div className={CapsuleStyles.large_button_ctx + ' ' + CapsuleStyles.noright} ref={btn_select}>
                        {open === 1 &&
                            <MenuFirst state={state} stateKey={stateKey} stateManager={stateManager} setOpen={setOpen}
                                       removeKeyParent={removeKeyParent} actionCallback={actionCallback}/>}
                        {open === 2 && <MenuEmoji stateKey={[...stateKey, 'emoji']} stateManager={stateManager}
                                                  passProps={passProps}/>}
                        {open === 3 &&
                            <MenuLabel closeLockRef={closeLockRef} state={state.label} stateKey={[...stateKey, 'label']}
                                       stateManager={stateManager} setOpen={setOpen}/>}
                        {open === 4 &&
                            <MenuLabel closeLockRef={closeLockRef} state={state.description || ""} nullable={true}
                                       stateKey={[...stateKey, "description"]} stateManager={stateManager}
                                       setOpen={setOpen}/>}
                        {open === 5 &&
                            <MenuLabel closeLockRef={closeLockRef} state={state.value} stateKey={[...stateKey, 'value']}
                                       stateManager={stateManager} setOpen={setOpen} placeholder="value"
                                       lockClose={false}/>}
                    </div>}
            </div>
        </DragLines>
...
function MenuFirst({state, stateKey, stateManager, setOpen, removeKeyParent, actionCallback}: {
    ...
    return <>
        ...
        <div className={CapsuleStyles.large_button_ctx_item} onClick={() => {
            ...
            <div className={CapsuleStyles.large_button_ctx_item_img}><img src={state.default ? DefaultActive : Default}
                                                                          alt=""/></div>
            <div
        ...
        <div className={CapsuleStyles.large_button_ctx_item} onClick={(ev) => {
            ...
            <div
                className={CapsuleStyles.large_button_ctx_item_text}>{state.emoji == null ? t('string-select.set-emoji') : t('string-select.change-emoji')}</div>
        </div>
        ...
        <div className={CapsuleStyles.large_button_ctx_item} onClick={(ev) => {
            ...
            <div className={CapsuleStyles.large_button_ctx_item_img}><img src={DescriptionText} alt=""/></div>
            <div
                className={CapsuleStyles.large_button_ctx_item_text}>{state.description == null ? t('string-select.add-description') : t('string-select.change-description')}</div>
        </div>
